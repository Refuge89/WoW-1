...
enum AttributesExF // Flags7
{
    ...
    FLAGS7_CAST_BY_CHARMER              = 0x40000,
};

enum AttributesExG // Flags8 // 7
{
...
static const uint32 g_spellSchoolConversionTable[SCHOOL_COUNT+1] =
{
	1,				// SCHOOL_NORMAL
	2,				// SCHOOL_HOLY
	4,				// SCHOOL_FIRE
...
enum SpellEffects
{
    ...
    SPELL_EFFECT_SUMMON_WILD,						//    41    SPELL_EFFECT_LEAP_41
    ...
    SPELL_EFFECT_SUMMON_CRITTER,							//    97 SPELL_EFFECT_SUMMON_MULTIPLE_TOTEMS
    ...
    SPELL_EFFECT_SUMMON_DEMON,                  //    112   SPELL_EFFECT_NONE_112
...
//    SPELL_FLAG_IS_HEALING_SPELL		= 0x00004000, // summons to inherit caster level or not
//    SPELL_FLAG_IS_HEALING_MANA_SPELL	= 0x00008000, // summons to inherit caster level or not
	SPELL_FLAG_IS_DISPEL_SPELL			= 0x00001000,
	SPELL_FLAG_CAN_BE_REFLECTED			= 0x00002000,
	SPELL_FLAG_IS_INHERITING_LEVEL		= 0x00004000,
	SPELL_FLAG_IS_MAXSTACK_FOR_DEBUFF	= 0x00008000,
...
{
    DIMINISHING_GROUP_BLIND_CYCLONE		= 0,  // 5
    DIMINISHING_GROUP_CHARM				= 1,  // 8
    DIMINISHING_GROUP_FEAR				= 2,  // 7
    DIMINISHING_GROUP_GOUGE_POLY_SAP	= 3,  // 6
    DIMINISHING_GROUP_HORROR			= 4,  // 10
    DIMINISHING_GROUP_KIDNEY_SHOT		= 5,  // 9
    DIMINISHING_GROUP_ROOT_PROC			= 6,  // 3
    DIMINISHING_GROUP_ROOT				= 7,  // 2
    DIMINISHING_GROUP_SLEEP				= 9,  // 4
    DIMINISHING_GROUP_BANISH			= 8,  // 11
    DIMINISHING_GROUP_STUN				= 10, // 0
    DIMINISHING_GROUP_STUN_PROC			= 11, // 1
...
{
	SpellTargetMod(uint64 _TargetGuid, uint8 _EffectMask, uint8 _HitResult) : TargetGuid(_TargetGuid), EffectMask(_EffectMask), HitResult(_HitResult)
	{
...
*/
		...
		void SpellEffectSummonWield(uint32 i);
	...
	//	std::map<uint64, Aura*> m_pendingAuras;
		...
		ARCEMU_INLINE bool hasAttribute(uint32 attribute) { return ((m_spellInfo->Attributes & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeEx(uint32 attribute) { return ((m_spellInfo->AttributesEx & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExB(uint32 attribute) { return ((m_spellInfo->AttributesExB & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExC(uint32 attribute) { return ((m_spellInfo->AttributesExC & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExD(uint32 attribute) { return ((m_spellInfo->AttributesExD & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExE(uint32 attribute) { return ((m_spellInfo->AttributesExE & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExF(uint32 attribute) { return ((m_spellInfo->AttributesExF & attribute) > 0); }
		ARCEMU_INLINE bool hasAttributeExG(uint32 attribute) { return ((m_spellInfo->AttributesExG & attribute) > 0); }

...
pSpellAura SpellAuraHandler[TOTAL_SPELL_AURAS] =
{
	...
	&Aura::SpellAuraPeriodicTriggerDummy,              //   SpellAuraDrinkNew,  //SpellAuraPeriodicTriggerDummy,//226 // used in brewfest spells, headless horseman, Aspect of the Viper
	...
	&Aura::SpellAuraRedirectThreat,//277 SPELL_AURA_REDIRECT_THREAT or SPELL_AURA_MOD_MAX_AFFECTED_TARGETS ?
...
void Aura::AddMod(uint32 t, int32 a, uint32 miscValue, uint32 i)
{
	...
	if(m_modcount >= 3)
	{
		LOG_DETAIL("Tried to add >3 (%u) mods to spellid %u [%u:%u, %u:%u, %u:%u]", m_modcount + 1, this->m_spellProto->Id, m_modList[0].m_type, m_modList[0].m_amount, m_modList[1].m_type, m_modList[1].m_amount, m_modList[2].m_type, m_modList[2].m_amount);
		return;
	}
...
void Aura::SpellAuraDummy(bool apply)
{
	...
	sLog.outDetail("Spell %u ( %s ) has an apply dummy aura effect, but no handler for it. ", m_spellProto->Id, m_spellProto->Name);
}
	...
	case 35696:
		{
			if ( m_target->IsPet() )
			{
				...
				if ( (PetOwner = TO_PET( m_target )->GetPetOwner()) != NULL )
				{
					...
					if( apply )
						mod->realamount = m_target->m_uint32Values[UNIT_FIELD_STAMINA] + m_target->m_uint32Values[UNIT_FIELD_INTELLECT];
...
/*
			if(!apply)
			{
				...
				{
					Player * pCaster = TO_PLAYER(m_target);
					Creature *summon = pCaster->m_eyeofacherus;
					pCaster->UnPossess();
					if(summon)
						delete summon;
					pCaster->m_eyeofacherus = 0;
					pCaster->SetUInt64Value(PLAYER_FARSIGHT, 0);
				}
			}
		}break;
...
//#endif
	...
	case 43872:
		{
				Unit* pCurrentTarget = m_caster->GetAIInterface()->getNextTarget();
				if (pCurrentTarget != NULL)
				{
					Unit* pTarget = pCurrentTarget;
					for (set<Object*  >::iterator itr = m_caster->GetInRangePlayerSetBegin(); itr !=  m_caster->GetInRangePlayerSetEnd(); ++itr) 
					{
					...
					//		continue;
						if (pPlayer->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FEIGN_DEATH))
							continue;
						if (pPlayer->isInRange(m_caster, 10.0f))
							continue;
						if (m_caster->GetAIInterface()->getThreatByPtr(pPlayer) >= m_caster->GetAIInterface()->getThreatByPtr(pCurrentTarget))
							continue;
					...
					if (pTarget == pCurrentTarget)
						m_caster->CastSpell(pTarget, dbcSpell.LookupEntryForced(26134), true);
				}
	...
	if ( TamingSpellid && ! GetTimeLeft() )
	{
		...
		{	
			...
			//	Pet* pPet = objmgr.CreatePet( m_spellInfo->EffectMiscValue[i] );
				pPet->SetInstanceID( m_caster->GetInstanceID() );
				pPet->CreateAsSummon( tamed->GetEntry(), tamed->GetCreatureInfo(), tamed, TO_PLAYER(m_caster), triggerspell, 2, 900000 );
...
void Aura::SpellAuraModStun(bool apply)
{
	...
	{ 
		...
		//	TO_PLAYER(m_target)->EventStunOrImmobilize();	
			TO_PLAYER(m_target)->EventStunOrImmobilize(caster, true);			
		}
	}
	else
	{
		...
		case FORM_CAT:
			{
				...
				else
				{
					...
					if(m_target->HasBuff(1850))
						m_target->RemoveAura(1850);//Dash rank1
					if(m_target->HasBuff(9821))
						m_target->RemoveAura(9821);//Dash rank2
					if(m_target->HasBuff(33357))
...
void Aura::SpellAuraMounted(bool apply)
{
	...
	if(apply)
	{
		...
		if(!isVehicleSpell && ci != NULL && ci->Male_DisplayID != 0)
			p_target->SetUInt32Value( UNIT_FIELD_MOUNTDISPLAYID , ci->Male_DisplayID);
		...
		if( p_target->m_CurrentVehicle != NULL ){
				((Vehicle*)p_target)->InitSeats( cp->vehicleid, TO_PLAYER(m_target) );
	...
	else
	{
		...
		//	p_target->m_CurrentVehicle->RemoveAllPassengers();
	...
	{   
		...
		if(TO_PLAYER(m_target)->GetSummon())
		{
			if(TO_PLAYER(m_target)->GetSummon()->GetUInt32Value(UNIT_CREATED_BY_SPELL) > 0)
				TO_PLAYER(m_target)->GetSummon()->Dismiss();				// warlock summon -> dismiss
			else
			{
				TO_PLAYER(m_target)->GetSummon()->Remove( true, true);	// hunter pet -> just remove for later re-call
		...
		if( TO_PLAYER(m_target)->GetShapeShift() && 
			...
			TO_PLAYER(m_target)->mountvehicleid = cp->vehicleid;

			if( TO_PLAYER(m_target)->mountvehicleid != 0 ){
			TO_VEHICLE(m_target)->InitSeats( cp->vehicleid, TO_PLAYER(m_target) );
			
			WorldPacket data( SMSG_PLAYER_VEHICLE_DATA, 12 );
			data << TO_PLAYER(m_target)->GetNewGUID();
			data << uint32( TO_PLAYER(m_target)->mountvehicleid );
			TO_PLAYER(m_target)->SendMessageToSet( &data, true );
			
			data.Initialize( SMSG_CONTROL_VEHICLE );
			TO_PLAYER(m_target)->GetSession()->SendPacket( &data );

			TO_PLAYER(m_target)->SetFlag( UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT );
			TO_PLAYER(m_target)->SetFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE );
		}
	}
	else
	{
		TO_PLAYER(m_target)->mountvehicleid = 0;
		TO_PLAYER(m_target)->m_bg--;
		TO_PLAYER(m_target)->m_MountSpellId = 0;
		TO_PLAYER(m_target)->flying_aura = 0;
		TO_PLAYER(m_target)->SetMount(0);

		if( !isVehicleSpell )
			m_target->SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID, 0);

		//if we had pet then respawn
		TO_PLAYER(m_target)->SpawnActivePet();
		TO_PLAYER(m_target)->RemoveAurasByInterruptFlag(AURA_INTERRUPT_ON_DISMOUNT);
	}
}*/
...
void Aura::SpellAuraPeriodicTriggerDummy(bool apply)
{
	...
	else
	{
		if(!sScriptMgr.CallScriptedDummyAura(m_spellProto->Id, mod->i, this, false))
			/* LOG_ERROR */ LOG_DEBUG("Spell %u ( %s ) has an apply periodic trigger dummy aura effect, but no handler for it.", m_spellProto->Id, m_spellProto->Name);
	}
}
...
void Aura::HandleAuraControlVehicle(bool apply)
{
	...
	if( apply ){
		if( m_target->m_CurrentVehicle != NULL )
			m_target->m_CurrentVehicle->AddPassenger( caster );

	}else{
		if( ( caster->m_CurrentVehicle != NULL ) && ( caster->m_CurrentVehicle == m_target->m_CurrentVehicle ) )
			m_target->m_CurrentVehicle->RemovePassenger( caster );		
	}
...
void Aura::SpellAuraPhase(bool apply)
{
	...
	if(m_spellProto->Id == 51852)
	{
		if ( m_target->IsPlayer() )
		{
			...
			if (m_target->GetUInt64Value(UNIT_FIELD_CHARM) && !apply)//remove other auras from charm on unapply
			{
				Creature * creat = ((Creature*)(m_target)->GetUInt64Value(UNIT_FIELD_CHARM));
				creat->RemoveAurasDueToSpell(GetSpellId());
			}
		}
	}
}

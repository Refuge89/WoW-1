...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
	...
	QueryResult *resultacct = RealmDatabase.Query("SELECT SUM(numchars) FROM realmcharacters WHERE acctid = '%d'", GetAccountId());
	if ( resultacct )
	{
	...
	RealmDatabase.Execute("DELETE FROM realmcharacters WHERE acctid= '%d' AND realmid = '%d'", GetAccountId(), sMaster.realmID);
    RealmDatabase.Execute("INSERT INTO realmcharacters (numchars, acctid, realmid) VALUES (%u, %u, %u)",  charcount, GetAccountId(), sMaster.realmID);
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket & recv_data)
{
	...
	//	AsyncQuery* q = new AsyncQuery(new SQLClassCallbackP1<World, uint32>(World::getSingletonPtr(), &World::_UpdateRealmCharCount, _accountId));
	//	q->AddQuery("SELECT COUNT(*) FROM characters WHERE acct = '%u'", _accountId);
	//	Database_Character->QueueAsyncQuery(q);
	}
	OutPacket(SMSG_CHAR_DELETE, 1, &fail);
}

uint8 WorldSession::DeleteCharacter(uint32 guid, bool updateRealmChars)
{
...
//		RealmDatabase.Execute("UPDATE realmcharacters SET numchars = numchars - 1 WHERE acctid = %d AND realmid = %d", _accountId, sMaster.realmID);
//	//	Database_Realm->Query("UPDATE realmcharacters SET numchars = numchars - 1 WHERE acctid = %d AND realmid = %d", _accountId, realmID);


		if(updateRealmChars) sWorld.UpdateRealmCharCount(_accountId);
		/* remove player info */
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket & recv_data)
{
	...
	RealmDatabase.Execute("UPDATE accounts SET online = 1 WHERE acct = '%u'", GetAccountId());
    RealmDatabase.Execute("UPDATE accounts SET active_realm_id = %d WHERE acct = '%u'", sMaster.realmID, GetAccountId());
...
void WorldSession::HandleCharacterCustomization( WorldPacket & recv_data )
{
	...
	data << uint8(E_RESPONSE_SUCCESS); // RESPONSE_SUCCESS
	data << uint64(guid);
	data << name;
	data << uint8(gender);
	data << uint8(skin);

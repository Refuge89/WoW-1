...
const char * gCreatureProtoFormat						= "uuuuuuufuuuffuuffuuuuuuuuuuffsuuufffuuuuuuuuuuuuuuuuu";
...
void ObjectMgr::LoadExtraCreatureProtoStuff()
{
	...
	QueryResult * result = WorldDatabase.Query( "SELECT entry,instance_mode,type+0,'',chance,maxcount,spell,spelltype+0,targettype+0,cooldown,floatMisc1,Misc2 FROM ai_agents" );
	...
	if( result != NULL )
	{
		...
		uint32 counter = 0;
		{
			do
			{
				...
				if( spe == NULL )
				{
					...
					Log.Warning("AIAgent", "Agent skipped, NPC %u tried to add non-existing Spell %u.", fields[0].GetUInt32(), fields[6].GetUInt32());
					continue;
				}
				...
				sp->spell = spe;
				sp->spellType = fields[7].GetUInt8();

				sp->spelltargetType = fields[8].GetUInt8();
				if( spe->c_is_flags & SPELL_FLAG_CASTED_ON_FRIENDS 
					&& !(sp->spelltargetType == TTYPE_OWNER))//just to make sure ;)
				...
				switch(sp->agent)
				{
					case AGENT_SPELL:
					{
						if(!sp->spell)
						{
							LOG_DEBUG("SpellId %u in ai_agent for %u is invalid.", (unsigned int)spellID, (unsigned int)sp->entryId);
							delete sp;
							sp = NULL;
							continue;
						}
						...
						{
							LOG_DEBUG("Teaching spell %u in ai_agent for %u", (unsigned int)spellID, (unsigned int)sp->entryId);
							delete sp;
							sp = NULL;
							continue;
						}
						...
						if(sp->cooldown == (uint32) - 1)
						{
							...
							{
								uint32 Casttime = 0; //most of the time 0
								uint32 Dur = 0;
								uint32 RecoveryTime = sp->spell->RecoveryTime;
								uint32 DurationIndex = sp->spell->DurationIndex;
								uint32 CastingTimeIndex = sp->spell->CastingTimeIndex;
								uint32 CategoryRecoveryTime = sp->spell->CategoryRecoveryTime;

								cooldown = GetCastTime(dbcSpellCastTime.LookupEntry(CastingTimeIndex));

								if(DurationIndex)
								{
									SpellDuration* sd = dbcSpellDuration.LookupEntry(DurationIndex);
									Dur =::GetDuration(sd);
								...
								if( CastingTimeIndex )
								{
									...
									uint32 maxRT = std::max(RecoveryTime, CategoryRecoveryTime);
									if (maxRT>cooldown)
										cooldown = maxRT;
								}
							}							
		...
		if(counter)
			Log.Success("AIAgent","Loaded %u ai_agents from database",counter);
		else
			Log.Error("AIAgent","No ai_agents found in database");
	}
	if(result)
		delete result;
}

...
bool ChatHandler::HandleReloadScriptsCommand(const char* args, WorldSession* m_session)
{
	...
	char msg[500];
	snprintf(msg, 500, "%sThe Lua Engine has been reloaded by %s.", MSG_COLOR_LIGHTBLUE, m_session->GetPlayer()->GetName());
	sWorld.SendWorldText(msg);
	sGMLog.writefromsession(m_session, "Lua Engine reloaded by %s", m_session->GetPlayer()->GetName());
	return true;
}
...
Spell::Spell(Object* Caster, SpellEntry* info, bool triggered, Aura* aur)
{
	...
	{
		Unit* u = u_caster->GetMapMgr()->GetUnit(u_caster->GetUInt64Value(UNIT_FIELD_CHARMEDBY));
		if (u)
		{
			u_caster = u;
			if (u->IsPlayer())
	...
	for(std::set<Object*>::iterator itr = m_caster->GetInRangeSetBegin(); itr != m_caster->GetInRangeSetEnd(); itr++)
	{
		...
		if(IsInrange(srcx, srcy, srcz, (*itr), r))
		{
			if(u_caster != NULL)
			{
				...
				{
					extended = DidHit(i, TO< Unit* >(*itr));
					if(did_hit_result != SPELL_DID_HIT_SUCCESS)
						ModeratedTargets.push_back(SpellTargetMod((*itr)->GetGUID(), extended, did_hit_result));
					else
...
uint8 Spell::prepare(SpellCastTargets* targets)
{
	...
	if( p_caster != NULL )
	{
		...
		if( GetGameObjectTarget() || GetProto()->Id == 21651)
		{
			...
			if( (GetProto()->Effect[0] == SPELL_EFFECT_OPEN_LOCK ||
				GetProto()->Effect[1] == SPELL_EFFECT_OPEN_LOCK ||
				GetProto()->Effect[2] == SPELL_EFFECT_OPEN_LOCK) && 
				p_caster->m_bg)
			{
...
void Spell::cast(bool check)
{
	...
	if(cancastresult == SPELL_CANCAST_OK)
	{
		...
		if(!(hasAttribute(ATTRIBUTE_ON_NEXT_ATTACK) && !m_triggeredSpell))  //on next attack
		{
			...
			if( u_caster )
				u_caster->SendPowerUpdate(false);
			...
			if(!IsReflected())
			{
						...
						if(GetProto()->Effect[x])
						{
							if(m_targetUnits[x].size())
							{
								for(i = m_targetUnits[x].begin(); i != m_targetUnits[x].end();)
								{
									...
									{
										// set target pointers
										HandleCastEffects(itr->TargetGuid, 0);

										// call effect handlers
										if(CanHandleSpellEffect(x, m_spellInfo->NameHash))
										{
											//Don't handle effect now
											if(GetProto()->Effect[x] != SPELL_EFFECT_SUMMON)
											{
												HandleCastEffects(itr->TargetGuid, x);
												effects_done[x] = true;
											}
											else
									...
									if( itr->HitResult == SPELL_DID_HIT_SUCCESS )
									{
										...
										if( unitTarget != NULL )
										{
											...
											if( GetProto()->TargetAuraState )
												unitTarget->RemoveFlag(UNIT_FIELD_AURASTATE, uint32(1) << (GetProto()->TargetAuraState - 1) );
				...
				for( uint32 x = 0; x < 3; ++x )
				{
					if(GetProto()->Effect[x])
					{
						...
						if(GetProto()->Effect[x] == SPELL_EFFECT_SUMMON_OBJECT && !GetProto()->always_apply)
						{
							break;
						}
					}
				...
				if( GetProto()->EffectApplyAuraName[0] != 0 ||
					GetProto()->EffectApplyAuraName[1] != 0 ||
...
void Spell::writeSpellMissedTargets(WorldPacket* data)
{
	...
	if(u_caster && u_caster->isAlive())
	{
		for(i = ModeratedTargets.begin(); i != ModeratedTargets.end(); ++i)
		{
			...
			*data << (*i).EffectMask;    // uint8
			*data << (*i).HitResult;    // uint8
...
void Spell::SendChannelUpdate(uint32 time)
{
	if(time == 0)
	{
		...
		//	DynamicObject* dynObj = u_caster->GetMapMgr()->GetDynamicObject(GET_LOWGUID_PART(guid));
			if(dynObj)
				dynObj->Remove();

		//	if(dynObj == NULL && u_caster->tmpAura.find(GetProto()->Id) == u_caster->tmpAura.end())  //no persistant aura or aura on caster
			if(dynObj == NULL && unitTarget->tmpAura.find(GetProto()->Id) == unitTarget->tmpAura.end())  //no persistant aura or aura on caster
			{
		...
		if(itr->second)
		{
			if(itr->second->GetSpellProto()->procCharges>0)
			{
				...
				//	Aura *aur = new Aura(itr->second->GetSpellProto(),itr->second->GetDuration(),itr->second->GetCaster(),itr->second->GetTarget(), m_triggeredSpell, i_caster);
					Aura* aur = sSpellFactoryMgr.NewAura(itr->second->GetSpellProto(), itr->second->GetDuration(), itr->second->GetCaster(), itr->second->GetTarget(), m_triggeredSpell, i_caster);
					Target->AddAura(aur);
				}
	...
	if(p_caster)
	{
		...
		for(uint8 i = 0; i < 3; ++i)
		{
			if( m_spellInfo->Effect[i] == SPELL_EFFECT_OPEN_LOCK && m_spellInfo->EffectMiscValue[i] == LOCKTYPE_SLOW_OPEN )
			{
				...
				{
					p_caster->HasBuff( p_caster->m_stealth ); // do7me
...
void Spell::SafeAddMissedTarget(uint64 guid)
{
	...
	ModeratedTargets.push_back(SpellTargetMod(guid, (*i).EffectMask, 2));
	}
}
...
void Spell::SafeAddModeratedTarget(uint64 guid, uint16 type)
{
	...
	ModeratedTargets.push_back(SpellTargetMod(guid, (*i).EffectMask, (uint8)type));
	}
}
...
//	uint32 status = spell->DiminishStatus;
	...
	assert(Grp <= DIMINISHING_GROUP_COUNT);

...
void Object::_BuildMovementUpdate(ByteBuffer* data, uint16 flags, uint32 flags2, Player* target)
{
	...
	if( IsVehicle() ){
		...
		if( u->m_CurrentVehicle != NULL )
			moveflags2 |= u->m_CurrentVehicle->GetMoveFlags2();
		...
		*data << (float)m_position.o;
		...
		if(flags2 & MOVEFLAG_TRANSPORT) //0x0200
		{
			...
			//	Unit* pUnit = TO_UNIT(this);
				...
				if (uThis->m_inVehicleSeatId != 0xFF  && vehicle->GetVehicleSeatEntry(uThis->m_inVehicleSeatId) != NULL )
				{
					*data << uThis->m_CurrentVehicle->GetNewGUID();
					*data << vehicle->GetVehicleSeatEntry(uThis->m_inVehicleSeatId)->attachmentOffsetX;
					*data << vehicle->GetVehicleSeatEntry(uThis->m_inVehicleSeatId)->attachmentOffsetY;
					*data << vehicle->GetVehicleSeatEntry(uThis->m_inVehicleSeatId)->attachmentOffsetZ;
					*data << float(0.0f);
					*data << uint32(0);
					*data << uThis->m_inVehicleSeatId;
				}
				else
				{
					*data << uint8(0);
			...
			{
				WoWGuid wowguid(pThis->GetSession()->GetMovementInfo()->transGuid);
				*data << wowguid;
				*data << pThis->GetSession()->GetMovementInfo()->transX << pThis->GetSession()->GetMovementInfo()->transY << pThis->GetSession()->GetMovementInfo()->transZ << pThis->GetSession()->GetMovementInfo()->transO;
				*data << pThis->GetSession()->GetMovementInfo()->transUnk << uint8(0);
			}
			else if(m_objectTypeId==TYPEID_UNIT && TO_CREATURE(this)->transporter_info.guid != NULL)
			{
				*data << TO_CREATURE(this)->transporter_info.guid;

				*data << uint32(HIGHGUID_TYPE_TRANSPORTER);
				*data << TO_CREATURE(this)->transporter_info.x << TO_CREATURE(this)->transporter_info.y << 
					TO_CREATURE(this)->transporter_info.z << TO_CREATURE(this)->transporter_info.o;
				*data << uint32(0);
				*data << uint8(0);
			}
/*			else if(pThis)
...
bool Object::SetPosition(float newX, float newY, float newZ, float newOrientation, bool allowPorting)
{
	...
	ARCEMU_ASSERT(!_isnan(newX) && !_isnan(newY) && !_isnan(newOrientation));
...
void Object::SetFlag( const uint32 index, uint32 newFlag )
{
	...
	if( index == UNIT_FIELD_AURASTATE )
	{
		...
		{
			for(uint32 i = 0; i < MAX_TOTAL_AURAS_END; ++i)
			{
...
void Object::SendMessageToSet(WorldPacket *data, bool bToSelf,bool myteam_only)
{
	...
	set<Object*  >::iterator itr = m_inRangePlayers.begin();
	set<Object*  >::iterator it_end = m_inRangePlayers.end();
	...
	if(myteam_only)
	{
		...
		if(gminvis && data->GetOpcode()!=SMSG_MESSAGECHAT)
		{
			...
			{
				ASSERT((p)->GetSession());
				if((p)->GetSession()->GetPermissionCount() > 0 && (*itr)->GetTeam()==myteam && PhasedCanInteract(*itr))
		...
		else
		{
			...
			{
				ASSERT((p)->GetSession());
				if((p)->GetTeam()==myteam && PhasedCanInteract(*itr))
					(p)->GetSession()->SendPacket(data);
			}
		}
	}
	else
	{
		...
		if(gminvis && data->GetOpcode()!=SMSG_MESSAGECHAT)
		{
			...
			{
				ASSERT((p)->GetSession());
				if((p)->GetSession()->GetPermissionCount() > 0 && PhasedCanInteract(*itr))
					(p)->GetSession()->SendPacket(data);
			}
		}
		else
...
bool Object::PhasedCanInteract(Object* pObj)
{
	...
	if( m_phase == ALL_PHASES || pObj->m_phase == ALL_PHASES ) // -1: All phases. But perhaps this should be limited to GameObjects?
		ret = true;

	if( pObj->m_phase & m_phase || pObj->m_phase == m_phase )
		ret = true;

...
SERVER_DECL DBCStorage<GameObjectDisplayInfoEntry> dbcGameObjectDisplayInfo;
...
SERVER_DECL DBCStorage<LiquidTypeEntry> dbcLiquidType;
...
SERVER_DECL DBCStorage <MapDifficultyEntry> sMapDifficultyStore; //(MapDifficultyEntryfmt); // only for loading
...
const char* ItemSetFormat = "usxxxxxxxxxxxxxxxxuuuuuuuuxxxxxxxxxuuuuuuuuuuuuuuuuuu";
...
const char* game_object_display_info_format = "nsxxxxxxxxxxffffffx";
...
//const char* MapDifficultyEntryfmt ="diisxxxxxxxxxxxxxxxxiix";
const char* MapDifficultyEntryfmt ="xiisxxxxxxxxxxxxxxxxiix";

uint32 GetLiquidFlags(uint32 liquidType)
{
    if (LiquidTypeEntry const* liq = dbcLiquidType.LookupEntry(liquidType))
        return 1 << liq->Type;
...
MapDifficulty const* GetDownscaledMapDifficultyData(uint32 mapId, Difficulty &difficulty)
{
    ...
    if (!mapDiff)
    {
        if (tmpDiff > MODE_HEROIC_25MEN) // heroic, downscale to normal
            tmpDiff -= 2;
        else
...
bool loader_stub(const char* filename, const char* format, bool ind, T & l, bool loadstrs)
{
	...
	return l.Load((sWorld.GetDataPath() + filename).c_str(), format, ind, loadstrs);
}
...
bool LoadDBCs()
{
	...
	LOAD_DBC("DBC/LiquidType.dbc", LiquidTypefmt, true, dbcLiquidType, true);
	...
	LOAD_DBC("DBC/GameObjectDisplayInfo.dbc", game_object_display_info_format, true, dbcGameObjectDisplayInfo, true);
	...
	LOAD_DBC("DBC/MapDifficulty.dbc", MapDifficultyEntryfmt, true, sMapDifficultyStore, true );
    // fill data

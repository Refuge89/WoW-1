...
{
	for (auto npc = npc_data.begin(); npc != npc_data.end(); ++npc)
    {
...
{
	for (auto gobj = gameobject_data.begin(); gobj != gameobject_data.end(); ++gobj)
    {
...
{
	for (auto npc = active_npcs.begin(); npc != active_npcs.end(); ++npc)
    {
        if (mEventScript != nullptr)
	...
	for (auto gameobject = active_gameobjects.begin(); gameobject != active_gameobjects.end(); ++gameobject)
    {
        if (mEventScript != nullptr)

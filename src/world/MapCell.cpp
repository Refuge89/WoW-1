...
void MapCell::SetActivity(bool state)
{
	if(!_active && state)
	{
		...
		//	CollideInterface.ActivateTile(_mapmgr->GetMapId(), _x / 8, _y / 8);
            ...
            if (m_celltilesLoaded[_mapmgr->GetMapId()][_x][_y] == 0)
            {
                mgr->loadMap((sWorld.GetDataPath()+ sWorld.vMapPath).c_str(), _mapmgr->GetMapId(), _x, _y);
                mmgr->loadMap((sWorld.GetDataPath()+ sWorld.mMapPath).c_str(), _mapmgr->GetMapId(), _x, _y);
            }
            ++m_celltilesLoaded[_mapmgr->GetMapId()][_x][_y];
	...
	else if(_active && !state)
	{
		...
		//	CollideInterface.DeactivateTile(_mapmgr->GetMapId(), _x / 8, _y / 8);
            ...
            MMAP::MMapManager* mmgr = MMAP::MMapFactory::createOrGetMMapManager();
            m_cellloadLock.Acquire();
            if (!(--m_celltilesLoaded[ _mapmgr->GetMapId()][_x][_y]))
            {
                mgr->unloadMap( _mapmgr->GetMapId(), _x, _y);
                mmgr->unloadMap( _mapmgr->GetMapId(), _x, _y);
            }
		...
		switch((*itr)->GetTypeId())
		{
			case TYPEID_UNIT:
				if( (*itr)->IsVehicle())
				{
					...
					TO< Vehicle* >(*itr)->m_respawnCell=NULL;
					delete TO< Vehicle* >(*itr);
				}
				else if(!(*itr)->IsPet())
				{

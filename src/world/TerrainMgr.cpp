...
/*
void TerrainTile::Load()  // working also
{
	//Normal map stuff
	std::string dataDir = "maps/";
	uint32 pathLen = static_cast<uint32>(dataDir.length() + strlen("%04u_%02u_%02u.map"));
	char *filename = new char[pathLen];
	snprintf(filename, pathLen, (dataDir + "%04u_%02u_%02u.map").c_str(), m_mapid, m_tx, m_ty);
...
{
	sLog.Debug("Terrain", "Loading %s", sWorld.GetDataPath() + filename);

	FILE* f = fopen((sWorld.GetDataPath() + filename).c_str(), "rb");

	if(f == NULL)
	{
		sLog.Error("Terrain", "%s does not exist", (sWorld.GetDataPath() + filename).c_str());
		return;
	}

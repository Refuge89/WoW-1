...
World::~World()
{
	Log.Warning("LocalizationMgr", "~LocalizationMgr()");
	sLocalizationMgr.Shutdown();

	Log.Warning("WorldLog", "~WorldLog()");
	delete WorldLog::getSingletonPtr();

	Log.Warning("ObjectMgr", "~ObjectMgr()");
	delete ObjectMgr::getSingletonPtr();

	Log.Warning("LootMgr", "~LootMgr()");
	delete LootMgr::getSingletonPtr();

	Log.Warning("LfgMgr", "~LfgMgr()");
	delete LfgMgr::getSingletonPtr();

	Log.Warning("ChannelMgr", "~ChannelMgr()");
	delete ChannelMgr::getSingletonPtr();

	Log.Warning("QuestMgr", "~QuestMgr()");
	delete QuestMgr::getSingletonPtr();

	Log.Warning("WeatherMgr", "~WeatherMgr()");
	delete WeatherMgr::getSingletonPtr();

	Log.Warning("TaxiMgr", "~TaxiMgr()");
	delete TaxiMgr::getSingletonPtr();

	Log.Warning("BattlegroundMgr", "~BattlegroundMgr()");
	delete CBattlegroundManager::getSingletonPtr();

	Log.Warning("InstanceMgr", "~InstanceMgr()");
	sInstanceMgr.Shutdown();

	//sLog.outString("Deleting Thread Manager..");
	//delete ThreadMgr::getSingletonPtr();
	Log.Warning("WordFilter", "~WordFilter()");
	delete g_chatFilter;
	...
	Log.Warning("SpellProcMgr", "~SpellProcMgr()");
	delete SpellProcMgr::getSingletonPtr();

	Log.Warning("SpellFactoryMgr", "~SpellFactoryMgr()");
...
void World::AddSession(WorldSession* s)
{
    ...
    if (m_playerLimit > 0)
    {
        float popu = GetSessionCount ();              //updated number of users on the server
        popu /= m_playerLimit;
        popu *= 2;
        RealmDatabase.Execute ("UPDATE realmlist SET population = '%f' WHERE id = '%d'", popu, sMaster.realmID);
...
//    uint32 realm_zone = 1;
//    Database_Realm->Execute("UPDATE realmlist SET icon = %u, timezone = %u WHERE id = '%d'", realmtype, realm_zone, realmID);
	...
	RealmDatabase.Execute("INSERT INTO uptime (realmid, starttime, startstring, uptime) VALUES('%u', " I64FMTD ", '%s', 0)", sMaster.realmID, uint64(start_time), isoDate);
    ...
    for (uint32 mapId = 0; mapId < dbcMap.GetNumRows(); mapId++)
        if (dbcMap.LookupEntry(mapId))
...
//    Log.Notice("GameObjectModel", "Loading GameObject models...");
    Log.Notice("GameObjectModel", "Loading GameObject models...");
	std::string vmapPath = sWorld.GetDataPath() + "vmaps";
	LoadGameObjectModelList(vmapPath);
//	vMapPath = sWorld.GetDataPath() + "vmaps";
//	LoadGameObjectModelList(vMapPath);
...
#undef MAKE_TASK
	...
	Log.Warning("World", "Object size: %u bytes", sizeof(Object));
	Log.Warning("World", "Unit size: %u bytes", sizeof(Unit) + sizeof(AIInterface));
	Log.Warning("World", "Creature size: %u bytes", sizeof(Creature) + sizeof(AIInterface));
	Log.Warning("World", "Vehicle size: %u bytes", sizeof(Vehicle) + sizeof(AIInterface));
	Log.Warning("World", "Player size: %u bytes", sizeof(Player) + sizeof(ItemInterface) + 50000 + 30000 + 1000 + sizeof(AIInterface));
	Log.Warning("World", "GameObject size: %u bytes", sizeof(GameObject));
...
// ------------------------------------------------------------------------------------------------
	...
	{
		Log.Warning("World", "Backgrounding loot loading...");
	...
	{
		Log.Warning("World", "Loading loot in foreground...");
		lootmgr.LoadLoot();
	}
...
void World::UpdateQueuedSessions(uint32 diff)
{
	...
	if(diff >= m_queueUpdateTimer)
	{
        ...
        uint32 tmpDiff = (m_gameTime - start_time);
        uint32 maxClientsNum = GetSessionCount();
        RealmDatabase.Execute("UPDATE uptime SET uptime = %u, maxplayers = %u WHERE realmid = %u AND starttime = " I64FMTD, tmpDiff, maxClientsNum, sMaster.realmID, uint64(start_time));
...
{
	Log.Warning("AddonMgr", "~AddonMgr()");
	sAddonMgr.SaveToDB();
	delete AddonMgr::getSingletonPtr();

	Log.Warning("AuctionMgr", "~AuctionMgr()");
	delete AuctionMgr::getSingletonPtr();
	Log.Warning("LootMgr", "~LootMgr()");
	delete LootMgr::getSingletonPtr();

	Log.Warning("MailSystem", "~MailSystem()");
...
void World::Rehash(bool load)
{
    ...
    std::string dataPath = Config.MainConfig.GetStringDefault("ServerData", "DataDir" , "");
    ...
    if (load)
    {
        if (dataPath != m_dataPath)
			sLog.outError("server.loading", "DataDir option can't be changed at world.conf reload, using current value (%s).", m_dataPath.c_str());
    }
    else
    {
        m_dataPath = dataPath;
        sLog.outDetail("server.loading", "Using DataDir %s", m_dataPath.c_str());
    }
...
//	Collision = Config.MainConfig.GetBoolDefault("Terrain", "Collision", 0);
...
{
// 	AsyncQuery* q = new AsyncQuery(new SQLClassCallbackP1<World, uint32>(World::getSingletonPtr(), &World::_UpdateRealmCharCount, AccountId));
 	AsyncQuery* q = new AsyncQuery(new SQLClassCallbackP1<World, uint32>(this, &World::_UpdateRealmCharCount, AccountId));
	q->AddQuery("SELECT COUNT(guid) FROM characters WHERE acct = '%u'", AccountId);
	CharacterDatabase.QueueAsyncQuery(q);
}

void World::_UpdateRealmCharCount(QueryResultVector & resultCharCount, uint32 AccountId)
{
	QueryResult* result = resultCharCount[0].result;
    if (result)
    {
		Field* fields = result->Fetch();
		uint32 charCount = fields[0].GetUInt32();
        delete result;
        RealmDatabase.Execute("DELETE FROM realmcharacters WHERE acctid= '%d' AND realmid = '%d'", AccountId, sMaster.realmID);
        RealmDatabase.Execute("INSERT INTO realmcharacters (numchars, acctid, realmid) VALUES (%u, %u, %u)", charCount, AccountId, sMaster.realmID);
    }
}
...
void World::PollMailboxInsertQueue(DatabaseConnection* con)
{
	...
	if(result != NULL)
	{
		Log.Warning("MailboxQueue", "Sending queued messages....");
		do
		{
			f = result->Fetch();
			...
			Log.Warning("MailboxQueue", "Sending message to %u (item: %u)...", f[1].GetUInt32(), itemid);
...
void World::SetPlayerLimit( int32 limit, bool needUpdate )
{
    ...
    if(db_update_need)
        Database_Realm->Execute("UPDATE realmlist SET allowedSecurityLevel = '%u' WHERE id = '%d'",uint8(GetPlayerLimit()), realmID);
}
*/
// cebernic:2008-10-19

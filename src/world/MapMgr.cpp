...
*/
	...
	Log.Warning("MapMgr", "Instance %u shut down. (%s)" , m_instanceID, GetBaseMap()->GetName());
}
...
void MapMgr::PushObject(Object* obj)
{
	...
	else
	{
		switch(obj->GetTypeFromGUID())
		{
			...
			case HIGHGUID_TYPE_VEHICLE:
				{
					ARCEMU_ASSERT((obj->GetUIdFromGUID()) <= m_VehicleHighGuid);
		...
		for (posX = startX; posX <= endX; ++posX )
		{
			for (posY = startY; posY <= endY; ++posY )
			{
				...
				if (cell)
				{
					...
					{
						obj = (*iter);
						if(!plr->IsVisible(obj->GetGUID()) && plr->CanSee(obj) && obj->GetDistance2dSq(X, Y) <= m_UpdateDistance)
						{
							ByteBuffer buf;
							count = obj->BuildCreateUpdateBlockForPlayer(&buf, plr);
							plr->PushCreationData(&buf, count);
...
Vehicle* MapMgr::CreateVehicle(uint32 entry)
{
	...
	v->Init();
	ARCEMU_ASSERT( v->GetTypeFromGUID() == HIGHGUID_TYPE_VEHICLE );
	m_VehicleStorage.insert( std::pair< const uint32, Vehicle* >(v->GetUIdFromGUID(), v));
	return v;
}
...
Creature* MapMgr::CreateCreature(uint32 entry)
{
	...
	cr->Init();
	ARCEMU_ASSERT( cr->GetTypeFromGUID() == HIGHGUID_TYPE_UNIT );
	return cr;
}

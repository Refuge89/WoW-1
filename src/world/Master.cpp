...
bool Master::Run(int argc, char** argv)
{
    ...
    // set server online
    RealmDatabase.Execute("UPDATE realmlist SET color = 0, population = 0 WHERE id = '%d'",sMaster.realmID);
	...
	wsport = Config.MainConfig.GetIntDefault("Listen", "WorldServerPort", DEFAULT_WORLDSERVER_PORT);
...
//	Log.Success("Server", "Ready for connections. Startup time: %ums", LoadingTime);
	Log.Success("Server:", "Ready for connections. Startup time: %u ms", GetMSTimeDiffToNow(startTime));
    ...
    // set server offline
    RealmDatabase.Execute("UPDATE realmlist SET color = 2 WHERE id = '%d'",sMaster.realmID);
...
bool Master::_StartDB()
{
    ...
    realmID = Config.MainConfig.GetIntDefault("RealmsDatabase", "RealmID", 0);
    if(!realmID)
    {
	...
	if(!RealmDatabase.Initialize(hostname.c_str(), (unsigned int)port, username.c_str(),
	                                 password.c_str(), database.c_str(), Config.MainConfig.GetIntDefault("RealmDatabase", "ConnectionCount", 5), 16384))
	{
		Log.Error("sql", "Main database initialization failed. Exiting.");
		return false;
	}

...
bool isHostile(Object* objA, Object* objB)
{
	...
	if( objA->IsUnit())
	{
		// Passengers inside a vehicle are not attackable
		// Mountable vehicles allow the passengers to be attacked however
		if(static_cast<Unit*>(objA)->m_CurrentVehicle)
			return false;
	}
    ...
    if( objB->IsUnit())
	{
		...
		if(objB->HasFlag(UNIT_FIELD_FLAGS, 2 | 128 | 256 | 65536))
	//	if(objB->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2 | UNIT_FLAG_UNKNOWN_8 | UNIT_FLAG_NOT_ATTACKABLE_9 | UNIT_FLAG_ALIVE))
			return false;

		// Passengers inside a vehicle are not attackable
		// Mountable vehicles allow the passengers to be attacked however
		if(static_cast<Unit*>(objB)->m_CurrentVehicle)
			return false;
	}
...
bool isAttackable(Object* objA, Object* objB, bool CheckStealth)
{
	...
	if(objA->IsUnit())
	{
		...
		// Mounted passengers on vehicles are not attackable
		if(static_cast<Unit*>(objA)->m_CurrentVehicle)
			return false;
	}

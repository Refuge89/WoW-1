...
void Player::Update(uint32 p_time)
{
	...
	{
		// Update our position
		float vposx = m_CurrentCharm->GetPositionX();
		float vposy = m_CurrentCharm->GetPositionY();
		float vposz = m_CurrentCharm->GetPositionZ();
		float vposo = m_CurrentCharm->GetOrientation();
		SetPosition(vposx, vposy, vposz, vposo, true);
	}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				Spell* spell = sSpellFactoryMgr.NewSpell(m_CurrentCharm, spellInfo, true, NULL);
...
void Player::SaveToDB(bool bNewCharacter /* =false */)
{
	...
	ss << recustomize_pending << ")";	// force_reset_talents
...
void Player::OnRemoveInRangeObject(Object* pObj)
{
	...
	if(pObj == m_CurrentCharm)
	{
		...
		if( m_CurrentCharm->m_temp_summon&&m_CurrentCharm->GetTypeId() == TYPEID_UNIT )
			TO_CREATURE( m_CurrentCharm )->DeleteMe();
...
void Player::EventTaxiInterpolate()
{
	...
	/*	m_CurrentTaxiPath->SetPosForTime(x, y, z, ntime - m_taxi_ride_time, &lastNode, m_mapId); // origional
	else
...
//	float traveldist = 0;
	float traveldist = uint32(path->getLength());
...
void Player::UnPossess()
{
	...
	data << pTarget->GetNewGUID() << uint8(1);
	m_session->SendPacket(&data);
...
/*
bool Player::UnPossessed(Unit* pTarget)
{

	if(m_tempSummon == pTarget)
	{
		m_tempSummon->RemoveFromWorld(false, true);
		if(m_tempSummon)
			m_tempSummon->DeleteMe();
...
/////////////////////////////////////////////////////////// Experience /////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(pVictim->GetCreatedByGUID() == 0 && !pVictim->IsPet() && pVictim->IsTagged())
		{
			...
			if(uTagger != NULL)
			{
                ...
                if (uTagger->IsPlayer())
                    pTagger = static_cast<Player*>(uTagger);

                if ((uTagger->IsPet() || uTagger->IsSummon()) && uTagger->GetPlayerOwner())
...
void Player::Phase(uint8 command, uint32 newphase)
{
	...
	if(m_CurrentCharm != NULL)
	{
		...
		m_CurrentCharm->Phase(command, newphase);
	}
}
...
/*
void Player::EnterVehicle( uint64 guid, uint32 delay ){
	if( delay != 0 ){
		sEventMgr.AddEvent( this, &Player::EnterVehicle, guid, uint32( 0 ), 0, delay, 1, 0 );
		return;
	}

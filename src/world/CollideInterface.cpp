...
void CCollideInterface::ActivateTile(uint32 mapId, uint32 tileX, uint32 tileY)
{
	...
	if(m_tilesLoaded[mapId][tileX][tileY] == 0)
	{
		mgr->loadMap((sWorld.GetDataPath()+ sWorld.vMapPath).c_str(), mapId, tileX, tileY);
		if (sWorld.Collision)
		{
			mmgr->loadMap((sWorld.GetDataPath()+ sWorld.mMapPath).c_str(), mapId, tileX, tileY);
		}
		else
...
void CCollideInterface::DeInit()
{
	Log.Warning("CollideInterface", "DeInit");
	//collision_shutdown();
}
...
void CCollideInterface::ActivateMap(uint32 mapid)
{
	...
	else
	{
        ...
        int count = fread(&params, sizeof(dtNavMeshParams), 1, f);
        fclose(f);
        if (count != 1)
        {
            Log.Debug("MMapManager", "MMAP:loadMapData: Error: Could not read params from file '%s'", filename);
            delete [] filename;
            return;
        }
...
//	int len = sWorld.GetDataPath().length() + strlen("mmaps/%04i%02i%02i.mmtile");
//	char* filename = new char[len];
//	sprintf(filename, (char *)(sWorld.GetDataPath() + "mmaps/%04i%02i%02i.mmtile").c_str(), mapId, tileX, tileY);
	std::string dataDir = sWorld.GetDataPath() + "mmaps/";
	uint32 len = static_cast<uint32>(dataDir.length() + strlen("%04u%02i%02i.mmtile"));
	char *filename = new char[len];
	snprintf(filename, len, (dataDir + "%04u%02i%02i.mmtile").c_str(), mapId, tileX, tileY);

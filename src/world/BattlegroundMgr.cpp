...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession* m_session, WorldPacket & pck)
{
	...
	if ( m_session->GetPlayer()->HasBuff(BG_DESERTER) && !m_session->HasGMPermissions() )
	{
	...
	Log.Warning("BattlegroundManager", "Player %u is now in battleground queue for instance %u", m_session->GetPlayer()->GetLowGUID(), (instance + 1));
...
CBattleground* CBattlegroundManager::CreateInstance(uint32 Type, uint32 LevelGroup)
{
	...
	if(IS_ARENA(Type))
	{
		...
		Log.Warning("BattlegroundManager", "Created arena battleground type %u for level group %u on map %u.", Type, LevelGroup, mapid);
...
//	localtime_s(&tm, &t);
	...
	mgr->m_battleground = bg;
	sEventMgr.AddEvent(bg, &CBattleground::EventCreate, EVENT_BATTLEGROUND_QUEUE_UPDATE, 1, 1, 0);
	Log.Warning("BattlegroundManager", "Created battleground type %u for level group %u.", Type, LevelGroup);
...
void CBattlegroundManager::HandleArenaJoin(WorldSession* m_session, uint32 BattlegroundType, uint8 as_group, uint8 rated_match)
{
	...
	if(as_group)
	{
		...
		else
		{
			...
			m_queueLock.Release();
			Log.Warning("BattlegroundMgr", "Group %u is now in battleground queue for arena type %u", pGroup->GetID(), BattlegroundType);
	...
	Log.Warning("BattlegroundMgr", "Player %u is now in battleground queue for {Arena %u}", m_session->GetPlayer()->GetLowGUID(), BattlegroundType);

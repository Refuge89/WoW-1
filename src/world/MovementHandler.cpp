...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data)
{
	...
	if(recv_data.GetOpcode() == MSG_MOVE_JUMP && _player->jumping == true && !GetPermissionCount() && !_player->m_eyeofacherus)
	{
	...
	if(!(HasGMPermissions() && sWorld.no_antihack_on_gm) && !_player->GetCharmedUnitGUID())
	{
		...
		if(sWorld.antihack_teleport && _player->m_position.Distance2DSq(movement_info.x, movement_info.y) > 3025.0f
		        && _player->m_runSpeed < 50.0f && !_player->transporter_info.guid  && !_player->m_eyeofacherus)
		{
	...
	else if(m_MoverWoWGuid.GetOldGuid() == _player->GetGUID())
	{

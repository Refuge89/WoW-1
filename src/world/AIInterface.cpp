	...
	if(m_AIState != STATE_EVADE)
	{
		...
		case EVENT_ENTERCOMBAT:
			{				
				...
				if( cr != NULL )
				{
					cr->HandleMonsterSayEvent(MONSTER_SAY_EVENT_ENTER_COMBAT);
/*
...
*/
				...
				// dismount if mounted
				if(m_Unit->IsCreature() && !(cr->GetCreatureInfo()->Flags1 & CREATURE_FLAG1_FIGHT_MOUNTED))
					m_Unit->SetMount(0);
				...
				if(pUnit->GetMapMgr() && pUnit->GetMapMgr()->GetMapInfo() && pUnit->GetMapMgr()->GetMapInfo()->type == INSTANCE_RAID)
				{
					...
					{
						if(cr->GetCreatureInfo()->Rank == ELITE_WORLDBOSS)
						{
		...
		case EVENT_LEAVECOMBAT:
			{
				...
				if(cr != NULL)
				{
					...
					cr->HandleMonsterSayEvent(MONSTER_SAY_EVENT_ON_COMBAT_STOP);

					if(cr->original_emotestate)
						m_Unit->SetEmoteState(cr->original_emotestate);
					else
				...
				else
				{
					...
					if(m_Unit->isAlive())
					{
						...
						{
							cr->UnTag();
							cr->SetUInt32Value(UNIT_DYNAMIC_FLAGS, cr->GetUInt32Value(UNIT_DYNAMIC_FLAGS) & ~(U_DYN_FLAG_TAGGED_BY_OTHER | U_DYN_FLAG_LOOTABLE));
						}
					}
				...
				{
					if(cr->m_spawn)
						m_Unit->SetMount(cr->m_spawn->MountedDisplayID);
					//m_Unit->SetMount(TO< Creature* >( m_Unit )->GetSpawnO->MountedDisplayID );
				}
				else if(cr!= NULL && m_Unit->GetMapMgr() )
				{
					m_Unit->GetMapMgr()->RemoveCombatInProgress(m_Unit->GetGUID());
				}
		...
		case EVENT_DAMAGETAKEN:
			{
				...
				if( cr!= NULL )
					cr->HandleMonsterSayEvent(MONSTER_SAY_EVENT_ON_DAMAGE_TAKEN);
...
/*				if(!modThreatByPtr(pUnit, misc1))
				{
					if( pUnit->IsPlayer() )
					{
						if( pUnit->mThreatRTarget )
							m_aiTargets.insert(TargetMap::value_type(TO_PLAYER(pUnit->mThreatRTarget)->GetGUID(), misc1));
						else
							m_aiTargets.insert(TargetMap::value_type(pUnit->GetGUID(), misc1));
					}
					else
				...
				if(!modThreatByPtr(m_Unit, misc1))
				{
					m_aiTargets.insert(TargetMap::value_type(pUnit->GetGUID(), misc1));
				}
				pUnit->CombatStatus.OnDamageDealt(m_Unit);
*/
		...
		case EVENT_FOLLOWOWNER:
			{
				...
				m_UnitToFollow = m_PetOwner->GetGUID();
				m_lastFollowX = m_lastFollowY = 0;
				FollowDistance = 4.0f;
		...
		case EVENT_UNFEAR:
			{
				...
				m_AIState = STATE_UNFEARED; // we need this to prevent permanent fear, wander, and other problems
	...
	switch(event)
	{
		case EVENT_UNITDIED:
		{
			...
			if( cr != NULL )
				cr->HandleMonsterSayEvent(MONSTER_SAY_EVENT_ON_DIED);
			            ...
			            || pInstance->m_mapInfo->type == INSTANCE_MULTIMODE))
			{
				...
				if(m_Unit->GetMapMgr() && m_Unit->GetMapMgr()->GetMapInfo() && m_Unit->GetMapMgr()->GetMapInfo()->type == INSTANCE_RAID)
				{
					if(cr != NULL)
					{
						...
						{
							m_Unit->GetMapMgr()->RemoveCombatInProgress(m_Unit->GetGUID());
						//	pUnit->GetMapMgr()->RemoveCombatInProgress(m_Unit->GetGUID());
						}
					}
				}
			}
...
void AIInterface::_UpdateTargets()
{
	...
	if( TO_CREATURE(m_Unit)->GetCreatureInfo() && TO_CREATURE(m_Unit)->GetCreatureInfo()->Type == UNIT_TYPE_CRITTER )
		return;
	...
	if( m_updateTargets )
	{
		...
		for(itr = m_aiTargets.begin(); itr != m_aiTargets.end();)
		{
			...
			if( ai_t->event_GetCurrentInstanceId() != m_Unit->event_GetCurrentInstanceId() || !m_Unit->PhasedCanInteract(ai_t) ||
				!ai_t->isAlive() || m_Unit->GetDistanceSq(ai_t) >= 6400.0f )
			{
	...
	if( m_nextTarget != NULL && m_nextTarget->isAlive() && m_AIState != STATE_EVADE && !m_Unit->IsCasting() )
	{
		...
		switch(agent)
		{
			case AGENT_MELEE:
			{
				...
				{
					if(m_UnitToFollow != NULL)
					{
						m_UnitToFollow = NULL; //we shouldn't be following any one
						m_lastFollowX = m_lastFollowY = 0;
						FollowDistance = 0.0f;
					}
						...
						if(infront)
						{
							...
							if(m_nextTarget != NULL)
							{
										...
										&& Rand(m_Unit->get_chance_to_daze(m_nextTarget)))
								{
									...
									if(fabs(our_facing-his_facing)<CREATURE_DAZE_TRIGGER_ANGLE && !m_nextTarget->HasAura(CREATURE_SPELL_TO_DAZE))
									{
		...
		case AGENT_SPELL:
			{
				...
				if (sWorld.Collision)
				{
					...
					los = mgr->isInLineOfSight(m_Unit->GetMapId(), m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_Unit->GetPositionZ(), m_nextTarget->GetPositionX(), m_nextTarget->GetPositionY(), m_nextTarget->GetPositionZ());
				}
...
//		SetNextTarget(FindTargetForSpell(m_nextSpell));
	...
	else if(m_nextTarget != NULL && !(m_nextTarget->m_phase & m_Unit->m_phase))     // the target or we changed phase, stop attacking
	{
		if(m_is_in_instance)
			setNextTarget(FindTarget());
		else
	...
	if(sWorld.Collision)
	{
		...
		if(m_Unit->GetMapMgr() != NULL && m_nextTarget != NULL)
		{
			if(!Flying())
			{
				float target_land_z = m_Unit->GetMapMgr()->GetLandHeight(m_nextTarget->GetPositionX(), m_nextTarget->GetPositionY(), m_nextTarget->GetPositionZ());

				if(fabs(m_nextTarget->GetPositionZ() - target_land_z) > _CalcCombatRange(m_nextTarget, false))
				{
					if(!m_nextTarget->IsPlayer())
					{
						if(target_land_z > m_Unit->GetMapMgr()->GetLiquidHeight(m_nextTarget->GetPositionX(), m_nextTarget->GetPositionY()))
							HandleEvent(EVENT_LEAVECOMBAT, m_Unit, 0);  //bugged npcs, probably db fault
					}
					else if(TO< Player* >(m_nextTarget)->GetSession() != NULL)
					{
						MovementInfo* mi = TO_PLAYER(m_nextTarget)->GetSession()->GetMovementInfo();
	...
	if(m_nextTarget != NULL && m_nextTarget->IsCreature() && m_AIState == STATE_EVADE)
		HandleEvent(EVENT_LEAVECOMBAT, m_Unit, 0);

	bool cansee = false;
	if(m_nextTarget)
	{
		if(m_nextTarget->event_GetCurrentInstanceId() == m_Unit->event_GetCurrentInstanceId())
		{
			if(m_Unit->IsCreature())
				cansee = TO< Creature* >(m_Unit)->CanSee(m_nextTarget);
			else
				cansee = TO< Player* >(m_Unit)->CanSee(m_nextTarget);
		}
		else
		{
			resetNextTarget();
	...
	if(cansee && m_nextTarget != NULL && m_nextTarget->isAlive() && m_AIState != STATE_EVADE && !m_Unit->IsCasting())
	{
		if( agent == AGENT_NULL || ( m_AIType == AITYPE_PET && !m_nextSpell ) ) // allow pets autocast
		{
			if(m_canFlee && !m_hasFleed && ( m_FleeHealth ? float(m_Unit->GetUInt32Value(UNIT_FIELD_HEALTH) / m_Unit->GetUInt32Value(UNIT_FIELD_MAXHEALTH)) < m_FleeHealth : 1))
...
/*								        && m_nextTarget->IsPlayer() && !m_Unit->IsPet() && health_before_strike > m_nextTarget->GetHealth()
								        && Rand(m_Unit->get_chance_to_daze(m_nextTarget)))
								{
									float our_facing = m_Unit->calcRadAngle(m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_nextTarget->GetPositionX(), m_nextTarget->GetPositionY());
									float his_facing = m_nextTarget->GetOrientation();
...
//					m_moveRun = false;
						...
						if(m_Unit->isAttackReady(false) && !m_fleeTimer)
						{
							...
							{
								m_Unit->setAttackTimer(0, false);
								SpellEntry* info = dbcSpell.LookupEntryForced(m_RangedAttackSpell);
								if(info)
								{
									Spell* sp = sSpellFactoryMgr.NewSpell(m_Unit, info, false, NULL);
									SpellCastTargets targets;
		...
		if(isHostile(m_Unit, caster))
		{
			...
			if( caster->IsPlayer() )
			{
				if( caster->mThreatRTarget )
					m_aiTargets.insert(TargetMap::value_type(TO_PLAYER(caster->mThreatRTarget)->GetGUID(), amount));
				else
...
Unit* AIInterface::FindHealTargetForSpell(AI_Spell *sp)
{
	...
	if(sp)
	{
		...
		if(healthPercent <= sp->floatMisc1 && !m_Unit->HasBuff(sp->spell->Id,m_Unit->GetGUID())) // Heal ourselves cause we got too low HP
		{
		...
		for(AssistTargetSet::iterator i = m_assistTargets.begin(); i != m_assistTargets.end(); ++i)
		{
			...
			if(healthPercent <= sp->floatMisc1 && !(*i)->HasBuff(sp->spell->Id,m_Unit->GetGUID()))
			{
...
{
	if( m_Unit->m_CurrentVehicle != NULL )
		return true;
	...
	if( nowtime > next_spell_time )
	{
		...
		else
		{
			...
			for(list<AI_Spell*>::iterator itr = m_spells.begin(); itr != m_spells.end(); ++itr)
			{
				...
				case STYPE_DEBUFF:
					{
						if (!m_nextTarget || m_nextTarget->HasBuff(sp->spell->Id))
							continue;
					}break;
				case STYPE_BUFF:
					{
...
bool AIInterface::modThreatByPtr(Unit* obj, int32 mod)
{
	...
	if( obj->mThreatRTarget && mod > 0)
	{
		...
		{
			TargetMap::iterator it = m_aiTargets.find(robj->GetGUID());
			if(it != m_aiTargets.end())
			{
				it->second += partmod;
				if((it->second + robj->GetThreatModifyer()) > m_currentHighestThreat)
				{
					// new target!
					if(!isTaunted)
					{
						m_currentHighestThreat = it->second + robj->GetThreatModifyer();
						setNextTarget(robj);
					}
				}
			}
			...
			{
				m_aiTargets.insert( make_pair( robj->GetGUID(), partmod ) );
				if((partmod + robj->GetThreatModifyer()) > m_currentHighestThreat)
				{
					if(!isTaunted)
					{
						m_currentHighestThreat = partmod + robj->GetThreatModifyer();
						setNextTarget(robj);
					}
				}
			}
...
{
	nextTarget = m_nextTarget->GetGUID();
...
void AIInterface::Event_Summon_Elemental(uint32 summon_duration, uint32 TotemEntry, int32 ResistanceType, uint8 Slot)
{
	...
	ourslave = ((Creature*)m_Unit->CreateTemporaryGuardian(TotemEntry,summon_duration,float(-M_PI*2), 0,Slot));
	if(ourslave != NULL)
	{

...
ObjectMgr::~ObjectMgr()
{
	Log.Warning("ObjectMgr", "Deleting Corpses...");
	CorpseCollectorUnload();

	Log.Warning("ObjectMgr", "Deleting Itemsets...");
	...
	Log.Warning("ObjectMgr", "Deleting PlayerCreateInfo...");
	...
	Log.Warning("ObjectMgr", "Deleting Level Information...");
	...
	Log.Warning("ObjectMgr", "Deleting Waypoint Cache...");
	...
	Log.Warning("ObjectMgr", "Deleting timed emote Cache...");
	...
	Log.Warning("ObjectMgr", "Deleting NPC Say Texts...");
	...
	Log.Warning("ObjectMgr", "Deleting Charters...");
	...
	Log.Warning("ObjectMgr", "Deleting Reputation Tables...");
	...
	Log.Warning("ObjectMgr", "Deleting Groups...");
	...
	Log.Warning("ObjectMgr", "Deleting Player Information...");
	...
	Log.Warning("ObjectMgr", "Deleting GM Tickets...");
	for(GmTicketList::iterator itr = GM_TicketList.begin(); itr != GM_TicketList.end(); ++itr)
		delete(*itr);

	Log.Warning("ObjectMgr", "Deleting Boss Information...");
	...
	Log.Warning( "ObjectMgr", "Cleaning up worldstate templates" );
	...
	if(result)
	{
		...
		do
		{
			...
			if(GetPlayerInfoByName(pn->name) != NULL)
			{
				...
				Log.Warning("ObjectMgr", "Renaming duplicate player %s to %s. (%u)", pn->name, temp, pn->guid);
			...
			if(!((++c) % period))
				Log.Warning("PlayerInfo", "Done %u/%u, %u%% complete.", c, result->GetRowCount(), c * 100 / result->GetRowCount());
		}
...
void ObjectMgr::LoadGuilds()
{
	...
	if(result)
	{
		...
		do
		{
			...
			if(!((++c) % period))
				Log.Warning("Guilds", "Done %u/%u, %u%% complete.", c, result->GetRowCount(), c * 100 / result->GetRowCount());
...
void ObjectMgr::SetHighestGuids()
{
	...
	Log.Warning("ObjectMgr", "HighGuid(CORPSE) = %u", m_hiCorpseGuid.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(PLAYER) = %u", m_hiPlayerGuid.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(GAMEOBJ) = %u", m_hiGameObjectSpawnId.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(UNIT) = %u", m_hiCreatureSpawnId.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(ITEM) = %u", m_hiItemGuid.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(CONTAINER) = %u", m_hiItemGuid.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(GROUP) = %u", m_hiGroupId.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(CHARTER) = %u", m_hiCharterId.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(GUILD) = %u", m_hiGuildId.GetVal());
	Log.Warning("ObjectMgr", "HighGuid(BUGREPORT) = %u", uint32(m_reportID.GetVal() - 1));
	Log.Warning("ObjectMgr", "HighGuid(TICKET) = %u", uint32(m_ticketid.GetVal() - 1));
	Log.Warning("ObjectMgr", "HighGuid(MAIL) = %u", uint32(m_mailid.GetVal()));
	Log.Warning("ObjectMgr", "HighGuid(EQUIPMENTSET) = %u", uint32(m_setGUID.GetVal() - 1));
}
...
void ObjectMgr::LoadVendors()
{
	...
	if(result != NULL)
	{
		...
		{
			Log.Warning("ObjectMgr", "Invalid format in vendors (%u/6) columns, not enough data to proceed.", result->GetFieldCount());
			delete result;
			return;
		}
		else if(result->GetFieldCount() > 6)
		{
			Log.Warning("ObjectMgr", "Invalid format in vendors (%u/6) columns, loading anyway because we have enough data", result->GetFieldCount());
		}
...
void ObjectMgr::LoadTotemSpells()
{
	...
	delete result;
	Log.Warning("ObjectMgr", "%u totem spells loaded.", m_totemSpells.size());
}
...
//					else if(Level>60)TotalManaGain+=Level+35;
	...
	Log.Warning("ObjectMgr", "%u level up information generated.", mLevelInfo.size());
}
...
void ObjectMgr::LoadCreatureTimedEmotes()
{
	...
	Log.Warning("ObjectMgr", "%u timed emotes cached.", result->GetRowCount());
	delete result;
}
...
void ObjectMgr::LoadCreatureWaypoints()
{
	...
	Log.Warning("ObjectMgr", "%u waypoints cached.", result->GetRowCount());
	delete result;
}
...
void ObjectMgr::RemoveCharter(Charter* c)
{
	...
	{
		Log.Warning("ObjectMgr", "Charter %u cannot be destroyed as type %u is not a sane type value.", c->CharterId, c->CharterType);
		return;
	}
...
void ObjectMgr::LoadReputationModifierTable(const char* tablename, ReputationModMap* dmap)
{
	...
	Log.Warning("ObjectMgr", "%u reputation modifiers on %s.", dmap->size(), tablename);
}
...
void ObjectMgr::LoadDisabledSpells()
{
	...
	Log.Warning("ObjectMgr", "%u disabled spells.", m_disabled_spells.size());
}
...
void ObjectMgr::LoadSpellTargetConstraints()
{
	...
	Log.Warning("ObjectMgr", "Loading spell target constraints...");
	...
	Log.Warning("ObjectMgr", "Loaded constraints for %u spells...", m_spelltargetconstraints.size());
}
...
{
    Log.Warning("ObjectMgr", "Loading Event Scripts...");
    ...
    else
    {
        if (!result)
        {
            Log.Warning("ObjectMgr", "Loaded 0 event_scripts. DB table `event_scripts` is empty.");
            return;
        }
    }
        ...
        if (itr->second.scripttype == static_cast<uint8>(EasyScriptTypes::SCRIPT_TYPE_GAMEOBJECT) || itr->second.scripttype == static_cast<uint8>(EasyScriptTypes::SCRIPT_TYPE_DUMMY))
        {
            ...
            case static_cast<uint8>(ScriptCommands::SCRIPT_COMMAND_ACTIVATE_OBJECT):
            {
                if ((itr->second.x || itr->second.y || itr->second.z) == 0)
                {
                    ...
                    if (static_cast<GameObject*>(target)->GetState() != 0)
                    {
                        static_cast<GameObject*>(target)->SetState(0);
                    }
                    else
                    {
                        static_cast<GameObject*>(target)->SetState(1);
                    }
                }
                else
                {

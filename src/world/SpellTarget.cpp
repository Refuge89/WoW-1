...
void Spell::FillTargetMap(uint32 i)
{
	...
	{
		if( !m_caster->IsVehicle()) // error !isAttackable(m_caster, obj, false)
		{
	...
	if(TargetType & (SPELL_TARGET_AREA | SPELL_TARGET_AREA_SELF))  //targetted aoe or
//	if(TargetType & (SPELL_TARGET_AREA ))
	...
	if (TargetType & SPELL_TARGET_OBJECT_CURTOTEMS && u_caster != NULL)
		for (uint32 i=1; i<5; ++i) //totem slots are 1, 2, 3, 4
			AddTarget(i, TargetType, u_caster->m_SummonSlots[i]);
...
void Spell::AddAOETargets(uint32 i, uint32 TargetType, float r, uint32 maxtargets)
{
	...
	if(TargetType & (SPELL_TARGET_AREA_PARTY | SPELL_TARGET_AREA_RAID) && !(p_caster == NULL && !m_caster->IsPet() && (!m_caster->IsCreature() && !m_caster->IsVehicle() || !m_caster->IsTotem())))
		return;
...
//	uint8 extended;
	...
	{
	//	uint8 extended2= DidHit(i, ((Unit*)m_caster));
		uint8 extended = 0;
		...
		ModeratedTargets.push_back(SpellTargetMod(obj->GetGUID(), extended, hitresult));
		return false;
	}
	else
	{
...
bool Spell::GenerateTargets(SpellCastTargets* t)
{
	...
	{
		SM_FFValue(u_caster->SM_PRadius,&r,m_spellInfo->SpellGroupType);
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        {
            if (m_spellInfo->SpellGroupType[3] & 0x00000100)// Starfall
                unMaxTargets = 2;
            break;
        }
        default:

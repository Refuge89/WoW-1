...
void Creature::SaveToDB()
{
	if(m_spawn == NULL)
	{
		...
		m_spawn->vehicle = GetProto()->vehicleid;
...
//	ss << (IsVehicle() ? TO_VEHICLE(this)->GetVehicleEntry() : 0) << " )";
	ss << (IsVehicle() ? TO_VEHICLE(this)->GetProto()->vehicleid : 0) << " )";
...
void Creature::Die(Unit* pAttacker, uint32 damage, uint32 spellid)
{
    ...
    if (pAttacker->IsVehicle())
    {
        ...
        if (vehicle_owner != NULL && vehicle_owner->IsPlayer())
        {
...
//	data << uint16( creature_info->Family );
	...
	for(std::vector<uint32>::iterator itr = proto->castable_spells.begin(); itr != proto->castable_spells.end(); ++itr)
	{

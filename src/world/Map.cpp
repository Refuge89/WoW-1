...
#define CREATURESPAWNSFIELDCOUNT 28
...
Map::~Map()
{
	Log.Warning("Map", "~Map %u", this->_mapId);
...
void Map::LoadSpawns(bool reload)
{
		...
		if(result)
		{
			if(CheckResultLengthCreatures(result))
			{
				do
				{
					...
					cspawn->vehicle = fields[27].GetInt32();
	...
	Log.Warning("Map", "%u creatures / %u gameobjects on map %u cached.", CreatureSpawnCount, GameObjectSpawnCount, _mapId);
}
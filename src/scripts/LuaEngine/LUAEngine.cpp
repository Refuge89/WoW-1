...
{
    LUALoadScripts rtn;
    Log.Success("LuaEngine", "Scanning Script-Directories...");
    ...
    Log.Success("LuaEngine", "Loading Scripts...");
    ...
    for (std::set<std::string>::iterator itr = rtn.luaFiles.begin(); itr != rtn.luaFiles.end(); ++itr)
    {
        ...
        else
        {
            ...
            {
                Log.Notice("LuaEngine", "loaded %s.", itr->c_str());
            }
        }
        cnt_uncomp++;
    }
    Log.Success("LuaEngine", "Loaded %u Lua scripts.", cnt_uncomp);
}

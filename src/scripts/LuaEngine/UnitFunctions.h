...
class LuaUnit
{
    ...
    static int AddItem(lua_State* L, Unit* ptr)
    {
        ...
        ItemPrototype const* item_proto = ItemPrototypeStorage.LookupEntry(id);
    ...
    static int TeleportUnit(lua_State* L, Unit* ptr)
    {
        ...
        if (!posX || !posY || !posZ || !mapId)
        {
            if (mapId)
            {
                Log.Debug("LuaEngineMgr", "LUATeleporter ERROR - Wrong Coordinates given (Map, X, Y, Z) :: Map%f%s%f%s%f%s%u", mapId, " X", posX, " Y", posY, " Z", posZ);
                return 0;
            }
            else
    ...
    {
        Log.Debug("LuaEngine", "GetTarget is outdated. Please use GetPrimaryCombatTarget.");
        TEST_PLAYER()
            Player* plr = static_cast<Player*>(ptr);
    ...
    static int CreatureHasQuest(lua_State* L, Unit* ptr)
    {
        ...
        Quest const* qst = QuestStorage.LookupEntry(questid);
    ...
    static int SendLootWindow(lua_State* L, Unit* ptr)
    {
        ...
        else if (guidtype == HIGHGUID_TYPE_GAMEOBJECT)
        {
            ...
            if (pGO != NULL && pGO->IsLootable())
            {
                ...
                {
                    default:
                        lootmgr.FillGOLoot(&ptr->loot, pGO->GetEntry(), pGO->GetMapMgr() ? (pGO->GetMapMgr()->iInstanceMode ? true : false) : false);
                        loot_type2 = 1;
                        break;
                    case 5:
                        lootmgr.FillSkinningLoot(&ptr->loot, pGO->GetEntry());
                        loot_type2 = 2;
                        break;
                }
            }
    ...
    */
    ...
    {
        TEST_UNIT();
        RET_NUMBER((uint32)ptr->GetAIInterface()->getMoveType());
        return 1;
    }
...
//            return 0;
    ...
    static int ExitVehicle(lua_State *L, Unit *ptr)
    {
        TEST_UNITPLAYER()
            ...
            if (ptr->m_CurrentVehicle != NULL)
                ptr->m_CurrentVehicle->RemovePassenger(ptr);
            else
                if (ptr->IsPlayer() && (ptr->m_CurrentVehicle != NULL))
    ...
    static int EjectAllVehiclePassengers(lua_State *L, Unit *ptr)
    {
        ...
        u->m_CurrentVehicle->RemoveAllPassengers();
    ...
    static int EjectVehiclePassengerFromSeat(lua_State *L, Unit *ptr)
    {
        ...
        u->m_CurrentVehicle->RemovePassenger(o);
...
/*    static int MoveVehiclePassengerToSeat(lua_State *L, Unit *ptr)
    ...
    //    uint32 seat = luaL_checkinteger(L, 2);
        ...
        u->m_CurrentVehicle->MovePassengerToNextSeat(passenger);

...
{
   Log.Warning("WorldPvP - Silithus", "removed aura created by spell %u", aura->GetSpellId());
   if(aura->GetSpellId() != SILITHYST_SPELL)
      return;

   if(aura->GetPlayerCaster())
      Log.Warning("WorldPvP - Silithus", "aura caster was a player");
   
   if(!aura->GetTarget()->IsPlayer())
   {   Log.Warning("WorldPvP - Silithus", "aura target was a player");
      return;}

...
public:
	...
	Player * GetRandomPlayerTarget()
	{
		...
		{ 
			if( (*iter) && TO_PLAYER(*iter)->isAlive() )
		...
		case SPELL_TARGET_LOWEST_THREAT:
			{
				...
				if( _unit->GetAIInterface()->getAITargetsCount() > 0 )
				{
					for(TargetMap::iterator itr = _unit->GetAIInterface()->GetAITargets()->begin(); itr != _unit->GetAIInterface()->GetAITargets()->end(); ++itr)
					{
						...
						Unit* ai_t = _unit->GetMapMgr()->GetUnit(itr->first);

						if( itr->second == 0 ) continue;
						if( lowest == 0 || ( itr->second + ai_t->GetThreatModifyer() ) < lowest )
						{
							lowest = ( itr->second + ai_t->GetThreatModifyer() ); 
							spelltarget = ai_t;
						}
					}
				}
	...
	void OnReachWP(uint32 i, bool usl)
	{
		if( i == 3 && check )
		{
			...
			if( Arthas && Jaina )//Show must go on!
			{
				...
				//2 = 2,5s
				TO_UNIT(_unit)->SendChatMessage(CHAT_MSG_MONSTER_SAY, (uint32)LANG_UNIVERSAL, 
				...
				//3 = 9s
				TO_UNIT(Arthas)->SendChatMessage(CHAT_MSG_MONSTER_SAY, (uint32)LANG_UNIVERSAL, 
				...
				//13 = 65s
				TO_UNIT(Jaina)->SendChatMessage(CHAT_MSG_MONSTER_SAY, (uint32)LANG_UNIVERSAL, 
		...
		case 2:
			{
				...
				if( c )
				{
					...
					TO_UNIT(c)->SendChatMessage(CHAT_MSG_MONSTER_SAY, (uint32)LANG_UNIVERSAL, 

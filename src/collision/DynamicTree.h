...
public:
    ...
    bool isInLineOfSight(float x1, float y1, float z1, float x2, float y2,
                         float z2, G3D::uint32 phasemask) const;

    bool getIntersectionTime(G3D::uint32 phasemask, const G3D::Ray& ray,
                             const G3D::Vector3& endPos, float& maxDist) const;

    bool getObjectHitPos(G3D::uint32 phasemask, const G3D::Vector3& pPos1,

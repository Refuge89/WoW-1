...
{
    G3D::uint32 treeSize = static_cast<G3D::uint32>(tree.size());
    size_t check = 0;
    ...
    G3D::uint32 count = static_cast<G3D::uint32>(objects.size());
...
bool BIH::readFromFile(FILE* rf)
{
    ...
    check += static_cast<G3D::uint32>(fread(&lo, sizeof(float), 3, rf));
    check += static_cast<G3D::uint32>(fread(&hi, sizeof(float), 3, rf));
    bounds = G3D::AABox(lo, hi);
    check += static_cast<G3D::uint32>(fread(&treeSize, sizeof(G3D::uint32), 1, rf));
    tree.resize(treeSize);
    check += static_cast<G3D::uint32>(fread(&tree[0], sizeof(G3D::uint32), treeSize, rf));
    check += static_cast<G3D::uint32>(fread(&count, sizeof(G3D::uint32), 1, rf));
    objects.resize(count); // = new G3D::uint32[nObjects];
    check += static_cast<G3D::uint32>(fread(&objects[0], sizeof(G3D::uint32), count, rf));
    return (G3D::uint64(check) == G3D::uint64(3 + 3 + 1 + 1 + G3D::uint64(treeSize) + G3D::uint64(count)));
}

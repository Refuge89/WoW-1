...
namespace VMAP
{
    ...
    {
        bool result = false;
        if (fwrite(&iTilesX, sizeof(G3D::uint32), 1, wf) == 1 &&
            fwrite(&iTilesY, sizeof(G3D::uint32), 1, wf) == 1 &&
            fwrite(&iCorner, sizeof(Vector3), 1, wf) == 1 &&
            fwrite(&iType, sizeof(G3D::uint32), 1, wf) == 1)
        {
            G3D::uint32 size = (iTilesX + 1) * (iTilesY + 1);
            if (fwrite(iHeight, sizeof(float), size, wf) == size)
            {
                size = iTilesX*iTilesY;
                result = fwrite(iFlags, sizeof(G3D::uint8), size, wf) == size;
            }
        }
    ...
    bool WmoLiquid::readFromFile(FILE* rf, WmoLiquid* &out)
    {
        ...
        if (fread(&liquid->iTilesX, sizeof(G3D::uint32), 1, rf) == 1 &&
            fread(&liquid->iTilesY, sizeof(G3D::uint32), 1, rf) == 1 &&
            fread(&liquid->iCorner, sizeof(Vector3), 1, rf) == 1 &&
            fread(&liquid->iType, sizeof(G3D::uint32), 1, rf) == 1)
        {
            ...
            if (fread(liquid->iHeight, sizeof(float), size, rf) == size)
            {
                ...
                result = fread(liquid->iFlags, sizeof(G3D::uint8), size, rf) == size;
            }
        }
    ...
    bool GroupModel::writeToFile(FILE* wf)
    {
        ...
        size_t chunkSize = sizeof(G3D::uint32)+ sizeof(Vector3)*count;
        if (result && fwrite(&chunkSize, sizeof(G3D::uint32), 1, wf) != 1)
            result = false;
        if (result && fwrite(&count, sizeof(G3D::uint32), 1, wf) != 1)
            result = false;
    ...
    bool WorldModel::writeFile(const std::string &filename)
    {
        ...
        if (result && fwrite("WMOD", 1, 4, wf) != 4)
            result = false;
        size_t chunkSize = sizeof(G3D::uint32) + sizeof(G3D::uint32);
        if (result && fwrite(&chunkSize, sizeof(G3D::uint32), 1, wf) != 1)
            result = false;
        if (result && fwrite(&RootWMOID, sizeof(G3D::uint32), 1, wf) != 1)
            result = false;

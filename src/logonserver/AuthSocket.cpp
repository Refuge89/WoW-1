...
void AuthSocket::HandleChallenge()
{
	...
	LOG_DEBUG("[AuthChallenge] got header, body is %u bytes", full_size);
	...
	{
		// wtf?
		LOG_DEBUG("[AuthChallenge] Client %s has wrong version. More up to date than server. Server: %u, Client: %u", GetRemoteIP().c_str(), LogonServer::getSingleton().max_build, m_challenge.build);
	...
	if(build < LogonServer::getSingleton().min_build)
	{
		...
		if(m_patch == NULL)
		{
			...
			LOG_DEBUG("[AuthChallenge] Client %s has wrong version. More out of date than server. Server: %u, Client: %u", GetRemoteIP().c_str(), LogonServer::getSingleton().min_build, m_challenge.build);
			SendChallengeError(CE_WRONG_BUILD_NUMBER);
			return;
		}
	...
	if(ipb != BAN_STATUS_NOT_BANNED)
		LOG_DEBUG("[AuthChallenge] Client %s is banned, refusing to continue.", GetRemoteIP().c_str());
...
{
	sInfoCore.SendRealms(this, m_account);

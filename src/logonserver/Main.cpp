...
void _OnSignal(int s)
{
	switch(s)
	{
		...
		case SIGHUP:
			{
				LOG_DEBUG("Received SIGHUP signal, reloading accounts.");
...
bool StartDb2()
{
	...
	// Note that the Config.RealmConfig.Get[$type] methods returns boolean value and not $type

	bool existsUsername = Config.RealmConfig.GetString("RealmDatabase", "Username", &lusername);
	bool existsPassword = Config.RealmConfig.GetString("RealmDatabase", "Password", &lpassword);
	bool existsHostname = Config.RealmConfig.GetString("RealmDatabase", "Hostname", &lhostname);
	bool existsName     = Config.RealmConfig.GetString("RealmDatabase", "Name",     &ldatabase);
	bool existsPort     = Config.RealmConfig.GetInt("RealmDatabase", "Port",     &lport);
	...
	if(!result)
	{
		...
		//  as experiments has seen other thread write to the console inbetween calls to sLog.outString(...)
		//  resulting in unreadable error messages.
		//If the <RealmDatabase> tag is malformed, all parameters will fail, and a different error message is given

		string errorMessage = "sql: Certain <RealmDatabase> parameters not found in " CONFDIR "\\Realms.conf \r\n";
		if(!(existsHostname || existsUsername || existsPassword  ||
		        existsName     || existsPort))
		{
			errorMessage += "  Double check that you have remembered the entire <RealmDatabase> tag.\r\n";
			errorMessage += "  All parameters missing. It is possible you forgot the first '<' character.\r\n";
		}
		else
		{
	...
	// Initialize it
	if(!Database_Realm->Initialize(lhostname.c_str(), (unsigned int)lport, lusername.c_str(),
	                          lpassword.c_str(), ldatabase.c_str(), Config.RealmConfig.GetIntDefault("RealmDatabase", "ConnectionCount", 5),
	                          16384))
	{
		LOG_ERROR("sql: Logon database initialization failed. Exiting.");
		return false;
...
bool Rehash()
{
	...
	string ips = Config.MainConfig.GetStringDefault("LogonLogonServer", "AllowedIPs", "");
	string ipsmod = Config.MainConfig.GetStringDefault("LogonLogonServer", "AllowedModIPs", "");
...
bool Rehash2()
{
	...
	if(!Config.MainConfig.SetSource(config_file2))
	{
		LOG_ERROR("Config file could not be rehashed.");
		return false;
	}
...
void LogonServer::Run(int argc, char** argv)
{
	...
	struct arcemu_option longopts2[] =
	{
		{ "checkconf",			arcemu_no_argument,				&do_check_conf,			1		},
		{ "screenloglevel",		arcemu_required_argument,		&screen_log_level,		1		},
		{ "fileloglevel",		arcemu_required_argument,		&file_log_level,		1		},
		{ "version",			arcemu_no_argument,				&do_version,			1		},
		{ "conf",				arcemu_required_argument,		NULL,					'r'		},
		{ 0, 0, 0, 0 }
	};
	...
	int r;
	while((r = arcemu_getopt_long_only(argc, argv, ":f:", longopts2, NULL)) != -1)
	{
		switch(r)
		{
			case 'r':
				/* Log filename was set */
				config_file = new char[strlen(arcemu_optarg)];
				strcpy(config_file2, arcemu_optarg);
				break;
			case 0:
				break;
			default:
	...
	if(do_check_conf)
	{
		...
		LOG_BASIC("Checking config file: %s", config_file2);
		if(Config.RealmConfig.SetSource(config_file2, true))
			LOG_BASIC("  Passed without errors.");
		else
...
*/
    ...
    sRealmList.Initialize(Config.RealmConfig.GetIntDefault("RealmsState","RealmsStateUpdateDelay", 20));
	...
	new PatchMgr;
	Log.Debug("AccountMgr", "Precaching accounts...");
	...
	int atime = Config.MainConfig.GetIntDefault("LogonRates", "AccountRefresh", 600);
	atime *= 1000;
	...
	// Load conf settings..
	uint32 cport = Config.MainConfig.GetIntDefault("LogonListen", "RealmListPort", 3724);
	uint32 sport = Config.MainConfig.GetIntDefault("LogonListen", "ServerPort", 8093);
	...
	string host = Config.MainConfig.GetStringDefault("LogonListen", "Host", "0.0.0.0");
	string shost = Config.MainConfig.GetStringDefault("LogonListen", "ISHost", host.c_str());
	...
	*/
	...
	string logon_pass = Config.MainConfig.GetStringDefault("LogonLogonServer", "RemotePassword", "r3m0t3b4d");
	Sha1Hash hash;

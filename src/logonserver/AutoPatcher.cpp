...
PatchMgr::PatchMgr()
{
	// load patches
#ifdef WIN32
	Log.Debug("PatchMgr", "Loading Patches...");
	...
	do
	{
		...
		Log.Debug("PatchMgr", "Found patch for %u locale `%s`.", srcversion, locality);
		pPatch = new Patch;
	...
	while(filecount--)
	{
		...
		Log.Debug("PatchMgr", "Found patch for b%u locale `%s` (%u bytes).", srcversion, locality, sb.st_size);
		pPatch = new Patch;
		size = sb.st_size;
		pPatch->FileSize = size;
		pPatch->Data = new uint8[size];

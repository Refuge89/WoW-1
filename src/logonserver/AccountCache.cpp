...
void AccountMgr::AddAccount(Field* field)
{
	...
	Database_Realm->Execute("INSERT INTO realmcharacters (realmid, acctid, numchars) SELECT realmlist.id, accounts.acct, 0 FROM realmlist,accounts LEFT JOIN realmcharacters ON acctid=accounts.acct WHERE acctid IS NULL");
...
void AccountMgr::UpdateAccount(Account* acct, Field* field)
{
	...
	if(id != acct->AccountId)
	{
        ...
        Database_Realm->Execute("DELETE FROM realmcharacters WHERE acctid='%d'", id);
		return;
	}
...
void IPBanner::Reload()
{
	...
	if(result != NULL)
	{
		do
		{
			...
			if(i == string::npos)
			{
				LOG_DEBUG("IP ban \"%s\" netmask not specified. assuming /32", ip.c_str());
			}
			else
				smask = ip.substr(i + 1);
...
void InformationCore::SendRealms(AuthSocket* Socket, Account* acc)
{
	realmLock.Acquire();
    ...
    QueryResult *result = sLogonSQL->Query("SELECT acct FROM accounts WHERE login = '%s'", acc->UsernamePtr->c_str());
    if(!result)
    {
        sLog.outError("[ERROR] user %s tried to login and we cannot find him in the database.", acc->UsernamePtr->c_str());
        delete result;
        return;
    }

//	acc->AccountId = (*result[0].Fetch()).GetUInt32();
	acc->AccountId = (*result).Fetch()[0].GetUInt32();
    delete result;
	...
	for(; itr != m_realms.end(); ++itr)
	{
		...
		QueryResult *result2 = Database_Realm->Query("SELECT numchars FROM realmcharacters WHERE realmid = '%u' AND acctid = '%u'",itr->first ,acc->AccountId);

        if( result2 )
        {
            Field *fields = result2->Fetch();
            AmountOfCharacters = fields[0].GetUInt8();
		//	AmountOfCharacters = (((*result2).Fetch()[2].GetUInt8()));
		//	AmountOfCharacters = ((*result2[2].Fetch()).GetUInt8());
		//	printf("result: %u \n",AmountOfCharacters);
            delete result2;
        }
        else		
		{
			sLog.outError("[ERROR] Realmid %u AnD to ACCOUNT &u we cannot find him in the database.", itr->first ,acc->AccountId);
			delete result;
			return;
		}
...
void InformationCore::CheckServers()
{
	...
	for(itr = m_serverSockets.begin(); itr != m_serverSockets.end();)
	{
		...
		{
			LOG_DEBUG("Disconnecting socket: %s due to it no longer being on an allowed IP.", s->GetRemoteIP().c_str());
			s->Disconnect();
		}
	}
...
void InformationCore::SetRealmOffline(uint32 realm_id)
{
	...
	if(itr != m_realms.end())
	{
		...
		Log.Debug("InfoCore", "Realm %u is now offline (socket close).", realm_id);
	}
	realmLock.Release();
}

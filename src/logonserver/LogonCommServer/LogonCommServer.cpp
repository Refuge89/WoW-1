...
void LogonCommServerSocket::HandleRegister(WorldPacket & recvData)
{
    ...
    if (my_id == -1)
    {
     //   my_id = sInfoCore.GenerateRealmID();
		...
		QueryResult *result2 = Database_Realm->Query("SELECT id FROM realmlist WHERE name = '%s'",Name.c_str());

        if( result2 )
        {
            Field *fields = result2->Fetch();
            my_id = fields[0].GetUInt8();
		}
        sLog.outString("First Registeration realm `%s` under ID %u.", Name.c_str(), my_id);
    }
    else
    {
		int new_my_id = my_id;
 //       sInfoCore.GetRealmIdByName(Name);
 ...
 //       Database_Realm->Execute ("UPDATE realmcharacters SET realmid = %u WHERE realmid = '%d'", my_id, new_my_id) &&
 //       Database_Realm->Execute ("UPDATE realmlist SET id = '%d' WHERE id = '%d'", my_id, new_my_id);
 //       my_id = new_my_id;
    }
...
//		Database_Realm->Execute("Replace INTO realmcharacters (realmid, acctid, numchars) SELECT realmlist.id, accounts.acct, 0 FROM realmlist,accounts LEFT JOIN realmcharacters ON acctid=accounts.acct WHERE acctid IS NULL");
//		Database_Realm->Execute("UPDATE realmcharacters SET numchars = %u WHERE acctid = %d AND realmid = %d", number_of_characters ,account_id, realm_id);
    }
